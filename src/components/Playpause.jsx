...
const playpause = () => {
  const hovered = () => {
    ...
    if (window.innerWidth <= 400) {
      ...
      const on_hovering_track_items2 =
        document.querySelector(".iamtop_playpause");
    ...
    <div className="iamplaypause hidden h-[100vh] w-full bg-[#000000] flex-col text-white items-start justify-between font-bold ">
      <div className="iamtop_playpause flex items-center justify-between w-full pt-3 p-10">
        ...
        <div onClick={hovered} className="track-items flex gap-5 font-bold ">
          <img
            className="track_image w-12 object-cover"
            src={track.image}
            alt=""
          />
          <div className="name_and_desc flex flex-col gap-2">
            <p className="track_name font-bold">{track.name}</p>
            <p className="track_desc text-xs text-[#d1c5ee]">
              {track.desc.slice(0, 32)}
            </p>
          </div>
        </div>
        ...
        <div className="seekbar_and_button flex flex-col items-center justify-center ">
      ...
      {/* // only for mobile */}
       ...
       <div className="mobile_content hidden">
        <div
          onClick={Unhovered}
          className="for_mobile_track_image gap-2 flex justify-start items-center flex-col mb-10 mt-7"
        >
          ...
          <img
            className="track_mobile_image w-[80vw] h-[47vh] object-cover"
            src={track.image}
            alt=""
          />
          <div className="gap-2 flex justify-start items-center flex-col">
            <p className="track_mobile_name font-bold text-[23px]">
              {track.name}
            </p>
            <p className="track_mobile_desc text-[12px] text-[#d1c5ee]">
              {track.desc.slice(0, 32)}
            </p>
          </div>
        </div>
        ...
        <div className="for_mobile_seekbar gap-5 not-only-of-type:flex flex-col items-center justify-center mb-5 ">
          <div className="button_contain flex justify-center items-center gap-7 font-bold text-4xl">
            <button onClick={previous}>
        ...
        <div className="mobile_iamseekbar w-full items-center justify-center flex gap-5">
          <p>
            {duration.currentduration.minutes}:
            {duration.currentduration.seconds}
          </p>
